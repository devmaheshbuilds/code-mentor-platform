...
const submitLesson = asyncHandler(async (req, res) => {
    ...
    if (testCasesResult.rows.length === 0) {
        throw new ApiError(404, "No test cases found for this lesson");
    }
    ...
    for (const testCase of testCasesResult.rows) {
        ...
        const actualOutput = (submitResponse.data.stdout || "").trim();
    ...
    if (existingProgress.rows.length > 0) {
        await pool.query(
            ...
            [allPassed ? "completed" : "in_progress", user_id, lesson_id]
        );
    } else {
        await pool.query(
            ...
            [user_id, lesson_id, allPassed ? "completed" : "in_progress"]
        );
    }

    return res
        .status(200)
        .json(new ApiResponse(200, { allPassed, results }, "Submission evaluated"));
